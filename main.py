...
def main(page: ft.Page):
    ...
    def create_task_row(task_id, task_text, completed):
        ...
        return ft.Row([task_checkbox, task_field, enable_button, save_button], alignment=ft.MainAxisAlignment.START)
    ...
    add_button = ft.IconButton(icon=ft.Icons.ADD_COMMENT, tooltip='Добавить продукт', on_click=add_task)

    filter_buttons = ft.Row(controls=[
        ft.ElevatedButton('моя карзина', on_click=lambda e: set_filter('all')),
        ft.ElevatedButton('купленое', on_click=lambda e: set_filter('completed')),
        ft.ElevatedButton("в корзине", on_click=lambda e: set_filter('uncompleted'))
    ], alignment=ft.MainAxisAlignment.SPACE_EVENLY)

    page.add(ft.Row(
        [task_input, add_button], alignment=ft.MainAxisAlignment.SPACE_BETWEEN
        ), filter_buttons, task_list)    
